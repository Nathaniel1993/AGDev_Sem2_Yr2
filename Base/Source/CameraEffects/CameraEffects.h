...
public:
	...
	Mesh* m_CameraEffects_Reticle;

	Mesh* m_CameraEffects_Ammo;
	...
	bool SetReticle(Mesh* aTarget);

	bool SetAmmoCount(Mesh* aTarget);

	// Get the BloodScreen
	Mesh* GetBloodScreen(void) const;

	Mesh* GetReticle(void) const;

	Mesh* GetAmmoCount(void) const;
	...
	bool SetStatus_Reticle(const bool m_bReticle);

	bool SetStatus_Ammo(const bool m_bAmmo);
